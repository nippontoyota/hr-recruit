...
def generate_candidate_summary_pdf(payload: dict) -> bytes:
    ...
    with pdf.table(
        ...
        for i in range(1, 6):
            row = table.row()
            pName = raw_data.get(f"prev{i}Name", 'LY Softwares' if i==1 else ('Suvan Technology Solutions' if i==2 else '0'))
            pFrom = raw_data.get(f"prev{i}From", '1-Jan-25' if i==1 else ('1-Nov-23' if i==2 else '0-Jan-00'))
            pTo = raw_data.get(f"prev{i}To", '15-Apr-26' if i==1 else ('25-Dec-24' if i==2 else '0-Jan-00'))
            pPos = raw_data.get(f"prev{i}Position", 'Customer support Executive & Telesales' if i==1 else ('HR Assistant & Admin' if i==2 else '0'))
            pReason = raw_data.get(f"prev{i}Reason", 'They changed the work location to another district' if i==1 else ('Better opportunity' if i==2 else '0'))
            pSal = raw_data.get(f"prev{i}Salary", '18000 + incentives' if i==1 else ('16,500' if i==2 else '0'))
            ...
            row.cell(s(pReason))
            row.cell(s(pSal))
            row.cell("Monthly" if i<=2 else "0")
    ...
    with pdf.table(
        ...
        row = table.row()
        row.cell("Interview Comments", rowspan=6, style=bold_bg_gray)
        row.cell("Jerry Jacob Mathew", style=bg_gray)
        row.cell("Had some CAC experience, good in communication, has the idea of the process, good in attitude. Can be considered.", style=bg_gray)
        row.cell("Grade", colspan=2, style=bold_bg_gray)
        row.cell("Marks (Max 10)", style=bold_bg_gray)
        
        row = table.row()
        row.cell("Wasim S", style=bg_gray)
        row.cell("Average communication, have Call Centre experience in Sales. Good attitude, have decent knowledge in Excel. Trainable can be considered.", style=bg_gray)
        row.cell("B", colspan=2, style=bold_bg_gray)
        row.cell("6", style=bold_bg_gray)
        
        row = table.row()
        row.cell("Jayan Mathew", style=bg_gray)
        row.cell("Good attitude. Average Communication. has some experience in call centre. Can be considered", style=bg_gray)
        row.cell("B", colspan=2, style=bold_bg_gray)
        row.cell("6", style=bold_bg_gray)
        ...
        row.cell("", style=bg_gray)
        row.cell("B", colspan=2, style=bold_bg_gray)
        row.cell("7", style=bold_bg_gray)
        
        row = table.row()
        row.cell("", style=bg_gray)
        row.cell("", style=bg_gray)
        row.cell("D", colspan=2, style=bold_bg_gray)
        row.cell("", style=bold_bg_gray)
        
        row = table.row()
        row.cell("CMD", style=bg_gray)
        row.cell("", style=bg_gray)
        row.cell("Total Marks", colspan=2, align="R", style=bold_bg_gray)
        row.cell("19", style=bold_bg_gray)
    ...
    with pdf.table(
        ...
        row.cell("Senior Executive")
        
        row = table.row()
        row.cell("Branch")
        row.cell("Kalamassery")
    ...
    with pdf.table(
        ...
        row.cell("Date: 04-07-2026", rowspan=2, align="R")
        
        row = table.row()
        row.cell("Post Applied", style=bold_style)
        row.cell("call center executive")
    ...
    with pdf.table(
        ...
        row.cell("Thrikkakara", colspan=2)
        
        row = table.row()
        row.cell("Name of the Councillor", style=bg_gray)
        row.cell("Rashid Ullapilly")
        ...
        row.cell("no police cases and no political background", colspan=3)

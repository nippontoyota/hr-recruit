...
export function PostFormStatus({ candidate, onUpdate }: PostFormStatusProps) {
  ...
  const status = candidate.post_form_status || 'NOT_SENT';
    ...
    return (
      <div className="py-8 w-full max-w-4xl mx-auto print-container">
        <div className="flex flex-col items-center mb-8 no-print">
          ...
          <p className="text-text-secondary mt-1">Post-interview responses from the candidate</p>
        </div>
        ...
        <div className="hidden print-header mb-8 border-b-2 border-black pb-6 font-sans">
          ...
          <div className="bg-gray-100 p-4 border border-black/20 rounded-lg">
            <h2 className="text-xl font-bold text-black uppercase tracking-wide mb-3 text-center border-b border-black/10 pb-2">Candidate Information Form</h2>
            <div className="flex justify-between text-sm font-medium">
              ...
              <div>
                ...
                <span className="text-black text-base">{candidate.post_form_submitted_at ? new Date(candidate.post_form_submitted_at).toLocaleDateString() : '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {formEntries.length > 0 ? (
          <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden print-no-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5 p-6 print:p-2 print:grid-cols-3 print:gap-x-4 print:gap-y-3 print-grid">
              {formEntries.map(([key, value]) => {
                ...
                return (
                  <div 
                    key={key} 
                    className={cn(
                      "flex flex-col gap-1 rounded-xl p-3.5 border border-border/30 bg-muted/5 hover:bg-muted/10 transition-colors print-item",
  ...
  return (
    <div className="py-8 max-w-2xl mx-auto w-full space-y-6">
      <div className="py-2">
        <div className="mb-4">
          <div>
            <h3 className="text-lg font-bold text-foreground">Candidate Information Form</h3>
            <p className="text-sm text-muted-foreground mt-0.5">
              {status === 'SENT' || status === 'VIEWED'
                ? 'Link generated. Share via WhatsApp using the preview on the right.'
                : 'Form link is ready to be generated for the candidate.'}
            </p>
          </div>
        </div>
        ...
        {candidate.post_share_url ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 w-full bg-background border border-border p-1.5 rounded-xl">
              <input
                type="text"
                readOnly
                value={candidate.post_share_url}
            ...
            {candidate.post_form_sent_at && (
              <p className="text-xs text-muted-foreground">
                Sent {new Date(candidate.post_form_sent_at).toLocaleString('en-IN')}
              </p>
            )}
            {(status === 'SENT' || status === 'VIEWED') && (
              <Button
          ...
          <div className="pt-2">
            <Button onClick={handleGenerateLink} disabled={isGenerating} className="gap-2">
              <Send className="w-4 h-4" />
              {isGenerating ? 'Generating...' : 'Generate Information Form'}
            </Button>
          </div>
        )}
      </div>
      ...
      <Modal
        ...
        title="Resend Candidate Information Form Link"
        size="lg"
      >

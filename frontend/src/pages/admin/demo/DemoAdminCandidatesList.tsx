...
export default function AdminCandidatesList() {
  ...
  const [candidates, setCandidates] = useState<any[]>(mockPipelineData.data);
  const [totalCount, setTotalCount] = useState(mockPipelineData.total_count);
  ...
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
        let filtered = mockPipelineData.data;
        if (stageFilter) {
            filtered = filtered.filter(c => c.current_stage === stageFilter);
        }
        if (searchQuery) {
            filtered = filtered.filter(c => c.full_name.toLowerCase().includes(searchQuery.toLowerCase()));
        }
